    ...
    def forward(self, backbone_output: BatchFeature, action_input: BatchFeature) -> BatchFeature:
        ...
        if self.config.use_multi_action_heads:
            if self.config.split_arm_heads:
                ...
                pred_claw = self.action_claw_decoder(model_output_actions, embodiment_id)
                # Structure: [left_arm(0-6), claw(7&15), right_arm(8-14)]
                pred_actions = torch.cat([pred_arm[:, :, : self.config.action_left_arm_dim], pred_claw[:, :, :1], pred_arm[:, :, self.config.action_left_arm_dim :], pred_claw[:, :, 1:2]], dim=-1)  # (B, T, action_dim)
                
                # Split ground truth velocity into corresponding parts (Structure: left_arm(0-6), claw(7&15), right_arm(8-14))
                velocity_arm = torch.cat([velocity[:, :, : self.config.action_left_arm_dim], velocity[:, :, (self.config.action_left_arm_dim + 1) : (self.config.action_left_arm_dim + 1 + self.config.action_right_arm_dim)]], dim=-1)  # (B, T, 14)
                velocity_claw = torch.stack([velocity[:, :, self.config.action_left_arm_dim], velocity[:, :, self.config.action_left_arm_dim + 1 + self.config.action_right_arm_dim]], dim=-1)  # (B, T, 2)
                ...
                action_mask_arm = torch.cat([action_mask[:, :, : self.config.action_left_arm_dim], action_mask[:, :, (self.config.action_left_arm_dim + 1) : (self.config.action_left_arm_dim + 1 + self.config.action_right_arm_dim)]], dim=-1)
                action_mask_claw = torch.stack([action_mask[:, :, self.config.action_left_arm_dim], action_mask[:, :, self.config.action_left_arm_dim + 1 + self.config.action_right_arm_dim]], dim=-1)
